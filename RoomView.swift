            ...
            VStack {
                ...
                if showHints {
                    if isEditingRoom {
                        ...
                        HintView(
                            title: editMode == .move ? "Move Mode" :
                                   editMode == .rotate ? "Rotate Mode" : "Scale Mode",
                            hints: editMode == .move ? [
...
struct RoomSceneView: UIViewRepresentable {
    ...
    class Coordinator: NSObject {
        ...
        @objc func handlePan(_ gesture: UIPanGestureRecognizer) {
            ...
            if isEditing && parent.selectedNode != nil {
                ...
                case .scale:
                    ...
                    let baseW = Float(selected.catalogItem?.defaultDimensions.width ?? 1) / 2
                    let baseD = Float(selected.catalogItem?.defaultDimensions.depth ?? 1) / 2
                    let newW = baseW * newScale.x
                    let newD = baseD * newScale.z
                    
                    let wallPad: Float = 0.05
                    
                    let minX = -roomWidth/2 + newW + wallPad
                    let maxX =  roomWidth/2 - newW - wallPad
                    let minZ = -roomLength/2 + newD + wallPad
                    let maxZ =  roomLength/2 - newD - wallPad
                    
                    if selected.position.x < minX || selected.position.x > maxX ||
                    selected.position.z < minZ || selected.position.z > maxZ {
        ...
        private func applyCollisionDetection(position: SCNVector3, furniture: FurnitureNode, scene: SCNScene) -> SCNVector3 {
            ...
            let baseWidth = Float(furniture.catalogItem?.defaultDimensions.width ?? 1.0) / 2.0
            let baseDepth = Float(furniture.catalogItem?.defaultDimensions.depth ?? 1.0) / 2.0
            let furnitureWidth = baseWidth * furniture.scale.x
            let furnitureDepth = baseDepth * furniture.scale.z
