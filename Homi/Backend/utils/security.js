...
const ACCESS_TOKEN_TTL = Number(process.env.ACCESS_TOKEN_TTL || 900); // 15 minutes
...
const JWT_HEADER_ENCODED = base64UrlEncodeBuffer(Buffer.from(JSON.stringify({ alg: "HS256", typ: "JWT" })));
...
export function createAccessToken(user) {
  return createSignedToken({ sub: user._id.toString(), type: "access" }, ACCESS_TOKEN_SECRET, ACCESS_TOKEN_TTL);
}

export function createRefreshToken(user) {
  return createSignedToken({ sub: user._id.toString(), type: "refresh" }, REFRESH_TOKEN_SECRET, REFRESH_TOKEN_TTL);
}
...
function createSignedToken(payload, secret, ttlSeconds) {
  ...
  const encodedPayload = base64UrlEncodeBuffer(Buffer.from(JSON.stringify({
    ...payload,
    iat: issuedAt,
    exp: expiresAt
  })));
